...
# 1. ATOMIC CONFIG: Less text = Faster generation
...
def get_optimized_image(path):
    ...
    img.thumbnail((500, 500)) # Smaller thumbnail = Faster upload
    img_byte_arr = io.BytesIO()
    img.save(img_byte_arr, format='JPEG', quality=40) # Lower quality for demo speed
...
async def main():
    ...
    # --- EVERYTHING BELOW THIS HAPPENS INSTANTLY IN MEMORY ---
    ...
    if count > 0:
        final_grade = total_score / count
        report_output += "-" * 40 + f"\nOVERALL EXAM GRADE: {final_grade:.1f}/10\n"
        
        # Local Remark Mapping (Zero Latency)
        if final_grade >= 8.5: rem = "Exceptional command of evidence."
        elif final_grade >= 7.0: rem = "Strong work; needs deeper analysis."
        else: rem = "Needs focus on core arguments and clarity."
        
        report_output += f"\n📝 FINAL REMARKS: {rem}\n" + "="*40

    # Print EVERYTHING at once to avoid console lag
